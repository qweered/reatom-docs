...
This operator adds `cacheAtom` property which is `MapAtom` from [@reatom/primitives](/packages/primitives#reatommap) and contains the cache of effect results. Do not change it manually! But you could use `reset` action to clear the cache. Also `cacheAtom` contains `invalidate` action which clears all existed cache and call new fetch with the last payload.
...
Do you need to implement a **pooling** pattern to stay your data fresh? Lets use `onConnect` from [@reatom/hooks](/packages/hooks) to control the neediness of this.
...
Also, `listResource` now has a `promiseAtom` that contains the last promise. You can use it with [useAtomPromise](/packages/npm-react/#use-atom-promise) in a React application, for example.
