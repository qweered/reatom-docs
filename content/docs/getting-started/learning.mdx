...
In the examples we will use the [@reatom/framework](/packages/framework/) package which is an umbrella package for the most common reatom packages.
You can install it with the following command

<Tabs groupId="package-manager" items={['npm', 'yarn', 'pnpm', 'bun']} persist>
  <Tab value="npm">```bash npm install @reatom/framework ```</Tab>
  <Tab value="yarn">```bash yarn add @reatom/framework ```</Tab>
  <Tab value="pnpm">```bash pnpm add @reatom/framework ```</Tab>
  <Tab value="bun">```bash bun add @reatom/framework ```</Tab>
</Tabs>
