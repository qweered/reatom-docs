...
<Tabs groupId="package-manager" items={['npm', 'yarn', 'pnpm', 'bun']} persist>
  <Tab value="npm">```bash npm install @reatom/logger ```</Tab>
  <Tab value="yarn">```bash yarn add @reatom/logger ```</Tab>
  <Tab value="pnpm">```bash pnpm add @reatom/logger ```</Tab>
  <Tab value="bun">```bash bun add @reatom/logger ```</Tab>
</Tabs>
